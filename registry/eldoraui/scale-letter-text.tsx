...
export default function ScaleLetterText({
  ...
  const suffixStartIndex = highlightSuffix ? text.lastIndexOf(highlightSuffix) : -1;
  ...
  return (
    <div className={className}>
      {text.split("").map((letter, index) => {
        const isHighlighted = suffixStartIndex !== -1 && index >= suffixStartIndex;
        return (
          <span
            key={index}
            className={`relative inline-block cursor-pointer select-none ${isHighlighted ? highlightClass : ""}`}
...
export function EscambioScaleLetterDemo() {
  return (
    <ScaleLetterText
      className="font-display text-center text-5xl font-black -tracking-widest text-white md:text-8xl md:leading-[6rem]"
      text="escamb.io"
      highlightSuffix=".io"
      highlightClass="text-cyan-400"
    />
